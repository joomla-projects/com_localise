...
class LocaliseModelPackage extends JModelItem
{
  ...
  protected function _populateState()
  {
    ...
    else
    {
      ...
      $this->setState('filename', $input->getCmd('filename', $cid[0]));
    }
    $this->setState('oldfilename', $input->getCmd('oldfilename', ''));
    $components = $input->get('components', array());
    $this->setState('components', $components);
    $modules = $input->get('modules', array());
    $this->setState('modules', $modules);
    $templates = $input->get('templates', array());
    $this->setState('templates', $templates);
    $plugins = $input->get('plugins', array());
