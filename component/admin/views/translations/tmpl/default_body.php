...
<?php foreach ($this->items as $i => $item) : ?>
	...
	<tr class="<?php echo $item->state; ?> row<?php echo $i % 2; ?>">
		...
		<td dir="ltr">
			...
			<?php endif; ?>
			<?php if ($item->writable && !$item->error && $canEdit && $have_raw_mode) : ?>
		...
		<td width="100" class="center" dir="ltr">
			...
			<?php elseif ($item->state == 'unexisting') : ?>
				<?php echo JHtml::_('jgrid.action', $i, '', array('tip'=>true, 'inactive_title'=>JText::sprintf('COM_LOCALISE_TOOLTIP_TRANSLATIONS_STATE_UNEXISTING', $item->translated, $item->unchanged, $item->total, $item->extra, $item->blocked, $item->untranslatable), 'inactive_class'=>'16-unexisting', 'enabled' => false, 'translate'=>false)); ?>
			<?php elseif ($item->tag == $reference) : ?>
				<?php echo JHtml::_('jgrid.action', $i, '', array('tip'=>true, 'inactive_title'=>JText::_('COM_LOCALISE_TOOLTIP_TRANSLATIONS_REFERENCE'), 'inactive_class'=>'16-reference', 'enabled' => false, 'translate'=>false)); ?>
			<?php elseif ($item->translated == $item->total || $item->complete) : ?>
				<?php echo JHtml::_('jgrid.action', $i, '', array('tip'=>true, 'inactive_title'=>JText::sprintf('COM_LOCALISE_TOOLTIP_TRANSLATIONS_COMPLETE', $item->translated, $item->unchanged, $item->total, $item->extra, $item->blocked, $item->untranslatable), 'inactive_class'=>'16-complete', 'enabled' => false, 'translate'=>false)); ?>
			<?php else : ?>
